...
def filter_images_with_small_segments_annotations(dataset_dicts):
    """
    ...
    def valid(x):
        ...
        new_anns = []
        for ann in anns:
            diag_len = np.sqrt((ann['bbox'][2])**2 +  (ann['bbox'][3])**2)
            if diag_len > 25:
                new_anns.append(ann)
        if len(new_anns)==0:
            return False
    ...
    dataset_dicts = [ x for x in dataset_dicts if x  ]
...
def filter_images_with_too_many_annotations(dataset_dicts):
    """
    Filter out images with none annotations or only crowd annotations
    (i.e., images without non-crowd annotations).
    A common training-time preprocessing on COCO dataset.
    ...
    def valid(x):
        ...
        new_anns = []
        if len(anns) > 49:
            sample_indices = random.sample(range(0,len(anns)),50 )
...
def get_detection_dataset_dicts(
    ...
    if filter_empty and has_instances:
        ...
        # dataset_dicts = filter_images_with_small_segments_annotations(dataset_dicts)
        # dataset_dicts = filter_images_with_too_many_annotations(dataset_dicts)
